...
class Student extends Admin_Controller
{
    ...
    //                             $student_data[$i][$fields[$j]] = "";
    //                         }

    //                         $student_data[$i]['admission_no'] = $this->encoding_lib->toUTF8($result[$i + 11][1]);
    //                         $student_data[$i]['roll_no'] = $this->encoding_lib->toUTF8($result[$i + 11][0]);
    //                         $student_data[$i]['firstname'] = $this->encoding_lib->toUTF8($result[$i + 11][3]);
    //                         $student_data[$i]['religion'] = $this->encoding_lib->toUTF8($result[$i + 11][2]);
    //                         $student_data[$i]['middlename'] = "";
    //                         $student_data[$i]['lastname'] = "";
    //                         $student_data[$i]['mobileno'] = $this->encoding_lib->toUTF8("No");
    //                         $student_data[$i]['gender'] = $this->encoding_lib->toUTF8($result[$i + 11][4]);
    //                         $student_data[$i]["email"] = $this->encoding_lib->toUTF8($result[$i + 11][1] . "@taalim.ma");
    //                         $student_data[$i]["guardian_email"] = $this->encoding_lib->toUTF8($result[$i + 11][1] . "@taalim.ma");
    //                         $student_data[$i]['measurement_date'] = '';
    //                         $student_data[$i]['admission_date'] = '';
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                if ($ext) {
                    ...
                    for ($k = 0; $k < $sheetCount; $k++) {
                        ...
                        $class_name = trim($result[6][2]);
                        $section_name = trim($sheetNames[$k]);

                        $is_exist_class = false;
                        $is_exist_section = false;
                        $is_exist_class_section = false;

                        // $class_id = "123";
                        // $section_id = "";

                        $row = $this->class_model->check_data_exists($class_name);
                        if (!$row) {
                            $this->class_model->add(['class' => $class_name, 'is_active' => 'yes']);
                            $row = $this->class_model->check_data_exists($class_name);
                        }
                        $class_id = $row->id;
                        $row = $this->db->select()->from('sections')->where('section', $section_name)->get()->row();
                        if (!$row) {
                            $section = ['section' => $section_name, 'is_active' => 'yes'];
                            $this->section_model->add($section);
                            $row = $this->db->select()->from('sections')->where('section', $section_name)->get()->row();
                        }
                        $section_id = $row->id;

                        $row = $this->classsection_model->check_data_exists(['section_id'=> $section_id,'class_id'=> $class_id]);
                        if (!$row) {
                            $this->db->insert('class_sections', ['class_id' => $class_id, 'section_id' => $section_id, 'is_active' => 'yes']);
                            // $row = $this->classsection_model->check_data_exists($class_name);
                        }
                        ...
                        if (!empty($result)) {
                            ...
                            for ($i = 0; $i <= count($result); $i++) {
                                ...
                                if (trim($this->encoding_lib->toUTF8($result[$i + 10][1])) == "" or $i > 60) {
                                    break;
                                }

                                $student_data = array();
                                $n = 0;
                                $student_data['roll_no'] = $this->encoding_lib->toUTF8($result[$i + 10][0]);
                                $student_data['admission_no'] = $this->encoding_lib->toUTF8($result[$i + 10][1]);
                                $student_data['lastname'] = $this->encoding_lib->toUTF8($result[$i + 10][2]);
                                $student_data['firstname'] = $this->encoding_lib->toUTF8($result[$i + 10][3]);
                                $student_data['gender'] = $this->encoding_lib->toUTF8($result[$i + 10][4]);
                                $student_data['dob'] = $this->encoding_lib->toUTF8($result[$i + 10][5]);
                                $student_data['religion'] = $this->encoding_lib->toUTF8($result[$i + 10][6]);
                                $student_data['middlename'] = "";
                                $student_data['mobileno'] = $this->encoding_lib->toUTF8("No");
                                $student_data["email"] = $this->encoding_lib->toUTF8($result[$i + 10][1] . "@taalim.ma");
                                $student_data["guardian_email"] = $this->encoding_lib->toUTF8($result[$i + 10][1] . "@taalim.ma");
                                $student_data['measurement_date'] = '';
                            ...
                            $this->session->set_flashdata('msg', '<div class="alert alert-success text-center">' . $this->lang->line('total') . ' ' . count($result) . $this->lang->line('records_found_in_l_file_total') . $rowcount . ' ' . $this->lang->line('records_imported_successfully') . '</div>');
                        } else {
    ...
    public function search()
    {
        ...
        $params = file('./system/database/parameters.json');
    ...
    public function manageReportParams()
    {
        if ($this->input->server('REQUEST_METHOD') != "POST")
            return;
        $community = $this->input->post('community');
        $delegation = $this->input->post('delegation');
        $name_academy = $this->input->post('nameAcademy');
        $year_s = $this->input->post('year_s');
        $name_delegation = $this->input->post('nameDelegation');
        $title = $this->input->post('title');
        $str = json_encode([
            'community' => $community,
            'delegation' => $delegation,
            'name_academy' => $name_academy,
            'year_s' => $year_s,
            'name_delegation' => $name_delegation,
            'title' => $title
        ]);
        $fp = fopen('./system/database/parameters.json', 'w');
        fwrite($fp, $str);
        fclose($fp);
        print ('success');
    }
    ...
    public function reportStudentList()
    {
        ...
        $params = implode('', file('./system/database/parameters.json'));
        $params = json_decode($params);
        $community = $params->community;
        $delegation = $params->delegation;
        $name_academy = $params->name_academy;
        $year_s = $params->year_s;
        $name_delegation = $params->name_delegation;
        $title = $params->title;
        $search_text = strtolower($search_text);
        $this->db->select('classes.class, sections.section, students.admission_no, students.firstname, students.lastname, students.roll_no, students.gender, students.dob, students.religion')->from('students');
        ...
        $this->db->where_in('students.id', $filter);
        $result = $this->db->get()->result();
        $height = 95 + 6.15 * (count($result) + 1);
        $orientation = 'P';
        if ($height < 210) $orientation = 'L';

        $pdf = new Pdf($orientation, PDF_UNIT, [210, $height], true, 'UTF-8', false);
        $pdf->setPrintHeader(false);
        $pdf->setPrintFooter(false);
        $pdf->setAutoPageBreak(true, 5);
        $pdf->AddPage();
        $pdf->Ln(5);
        $pdf->Image('http://localhost/backend/images/mark.png', 50, 12, 100, 12, 'PNG', '', 0, false, 150, 10, false, false, 0, false, false, false);
        $pdf->Ln(15);
        $pdf->SetFont('aefurat', 'B', 12);
        $pdf->Line(10, 40, 200, 40, ['B']);
        $pdf->SetFont('aefurat', 'B', 22);
        $pdf->Cell(0, 15, $title, 0, 1, 'C', 0, '1', 0, false, 'M', 'M');
        $pdf->Ln(6);
        $pdf->SetFont('aefurat', 'B', 14);

        $nom_prenom = 'المديرية الاقليمية ';
        $date_naiss = 'سوس ماسة ';
        $academy = 'الأكاديمية';
        $delegation = 'المديرية';
        $section = 'المستوى ';
        ...
        $school = 'المؤسسة';
        $comun = 'الجماعة';
        $yearschoolar = ' الموسم الدراسي ';
        $classes11 = $result[0]->class;
        $section11 = $result[0]->section;
        $tbl = <<<EOD
        <table border="0" cellpadding="2" cellspacing="1">
        <tr>
        <td width="35%" style="text-align:left;vertical-align:middle;font-weight:bold;">$community</td>
        <td width="20%" style="text-align:right;vertical-align:middle;font-weight:bold;">$comun</td>
        <td width="30%" style="text-align:right;vertical-align:middle;font-weight:bold;">$name_academy</td>
        <td width="15%" style="text-align:right;vertical-align:middle;font-weight:bold;">$academy </td>
        </tr>
        <tr>
        <td style="text-align:left;vertical-align:middle;font-weight:bold;">$name_delegation</td>
        <td style="text-align:right;vertical-align:middle;font-weight:bold;">$school</td>
        <td style="text-align:right;vertical-align:middle;font-weight:bold;">$delegation</td>
        <td style="text-align:right;vertical-align:middle;font-weight:bold;">$delegation</td>
        </tr>
        <tr>
        <td style="text-align:left;vertical-align:middle;">$year_s</td>
        <td style="text-align:right;vertical-align:middle;">$yearschoolar</td>
        <td style="text-align:right;vertical-align:middle;">$section11</td>
        ...
        <td style="text-align:center;vertical-align:middle;font-weight:bold;"> </td>
        <td style="text-align:right;vertical-align:middle;font-weight:bold;">$classes11</td>
        <td style="text-align:right;vertical-align:middle;font-weight:bold;">$classes </td>
        </tr>
        </table>
        EOD;
        ...
        $w = [45, 35, 15, 45, 28, 20];
        $headers = ['Religion ', 'Birthday', 'Gender', 'Name', 'Admission No', 'No'];
        ...
        foreach ($result as $row) {
            if ($search_text !== '' && !str_contains(strtolower(implode((array) $row)), $search_text))
                continue;
            $pdf->Cell($w[0], 6, $row->religion, 'LR', 0, 'C', $fill);
            $pdf->Cell($w[1], 6, $row->dob, 'LR', 0, 'C', $fill);
            $pdf->Cell($w[2], 6, $row->gender, 'LR', 0, 'C', $fill);
            $pdf->Cell($w[3], 6, $row->firstname . ' ' . $row->lastname, 'LR', 0, 'C', $fill);
            $pdf->Cell($w[4], 6, $row->admission_no, 'LR', 0, 'C', $fill);
            $pdf->Cell($w[5], 6, $count, 'LR', 0, 'C', $fill);
            $pdf->Ln();
            $fill = !$fill;
            $count++;
        }
        $pdf->Cell(array_sum($w), 0, '', 'T');
        $file_content = $pdf->Output('list_students.pdf', 'S');
        header('Content-Type: application/pdf');
        header('Content-Disposition: attachment; filename="list_students.pdf"');
        echo $file_content;
        exit;
    }
    ...
    public function dtstudentlist()
    {
        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
                ...
                $row = array();
                $row[] = '<div align="center"><input type="checkbox" value="' . $student->id . '" checked></div>';
